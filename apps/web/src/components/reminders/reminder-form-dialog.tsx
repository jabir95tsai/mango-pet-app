...
function ReminderForm({
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    if (!title.trim()) {
      setError(tR("errors.titleRequired"));
      return;
    }
    const triggerDate = new Date(triggerAt);
    if (!triggerAt || Number.isNaN(triggerDate.getTime())) {
      setError(tR("errors.invalidTime"));
      return;
    }
    setSaving(true);
    setError(null);
    try {
      ...
      setError(err instanceof Error ? err.message : tR("errors.saveFailed"));
    } finally {
      setSaving(false);
    }
  }
    ...
    <form onSubmit={handleSubmit} className="flex flex-col gap-4">
      <div className="flex flex-col gap-1">
        ...
        <Input
          ...
          placeholder={tR("placeholders.title")}
          required
        />
      </div>
