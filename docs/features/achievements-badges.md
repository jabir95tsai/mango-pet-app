...
## ✅ UI/UX 已交付 — §H 解鎖慶祝 + §H.1（Web/PWA UI/UX session 2026-06-03 — commit 618ffb0 on main）

**Shipped to main → App Hosting（prod live + Chrome MCP 驗過，真 guest）。** 純前端慶祝層 + 首開 hook，未碰 Backend / grant / 成就頁既有授予邏輯。

### 做了什麼
- **偵測 hook**（`lib/achievement-celebration.ts` + `components/achievements/celebration-provider.tsx`，掛 `/app` 層 overlay）：realtime grant listener diff 一個 per-user localStorage「已慶祝集合」（key `mango.celebratedAchievements.{uid}`）；`?unlocked=<id,…>`（逗號多枚）deep-link 直接開 modal 後 strip query。慶祝後把 id 寫回集合 → 不重播。
- **§H.1 首開行為**：已慶祝集合**起始為空、不靜默 seed**。既有使用者首開 → diff = 全部當前 grants（含 backfill）→ 跳一次慶祝；全新 0 徽章使用者 grants 為空 → 自然不跳。
- **§H.1 大量 N**（門檻 `SUMMARY_THRESHOLD=6`）：1 枚=單枚 modal；2–6 枚=變體 B（縮圖 strip + dots 翻看）；>6 枚=彙總「你已獲得 N 枚成就！」+ 疊放 5 枚金幣 + 「+N」chip。
- **Modal**（`celebration-modal.tsx`，像素對齊設計稿）：scrim+blur、芒果金幣獎章（gradient/shadow/medal-pop 取自設計 bundle 的 celebration-screen）、glow/rays、kicker「🎉 恭喜解鎖！」、name/desc/category chip、主鈕「太棒了」(關) / 次鈕「查看所有成就」(→`/app/achievements`)。
- **⭐ 共用 confetti**：紙花改用 confetti-engine-upgrade 的 `<ConfettiCanvas>`（新增 `portal` prop，modal 內 inline 渲染於卡片之後）；reduced-motion 直接不渲染 confetti（變體 C）+ globals.css `.celebrate-*` 動畫硬停 + glow 靜態。
- **i18n**：`Achievements.celebrate.summaryTitle` / `.summaryBody`（zh-TW + en），其餘沿用既有 celebrate.* + `Achievements.{id}.*`。

### Prod 驗證（Chrome MCP，真 guest uid ddY7…Vrl2）
①`?unlocked=walk-1`→單枚「初次遛狗」✅；②7 ids→彙總「你已獲得 7 枚成就！」+ 疊放 5 + 「+2」✅；3 ids→變體 B（縮圖 strip + dots）✅；§H.1 首開→關掉 deep-link 批次後自動跳該 guest 真實未慶祝 grant（多寵之家/pet-3）✅；④全部慶祝後 localStorage 寫入 `[walk-1,pet-1,dist-5,pet-3]`、重進頁無 modal（不重播）✅；③reduced-motion 由 `!reduced` gate（無 confetti）+ CSS media query（動畫硬停、glow 靜態）雙重保證（harness 無法 live emulate，code-verified）。

### 注意 / handoff
- ⚠️ confetti canvas 用 `requestAnimationFrame`，在**背景分頁**（headless 驗證）會被瀏覽器暫停 → 自動化截不到紙花動畫；modal 本體（DOM/CSS）正常顯示。前景真實使用者紙花會正常播放。
- 慶祝 modal 與 walk 達標彩帶現在**共用同一個 `<ConfettiCanvas>`**（confetti-engine-upgrade.md ⭐ 整併建議已達成）。
