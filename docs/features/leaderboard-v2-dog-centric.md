...
> **角色分工**：Backend（公式 + 聚合 + rules + migration）✅ 已交付 → **UI/UX（端到端狗榜 UI：頁/tab/設定 + 視覺/a11y/響應式/動效）**。
> ⚠️ **路由更新（user 2026-05-30）**：UI 由 **UI/UX 角色**負責（非 Feature Builder）。後端已就緒，UI/UX 此次做的是 net-new surface（含資料 wiring + 視覺），範圍比平常的「polish 既有畫面」廣。
...
- **→ UI/UX（user 2026-05-30 指定，端到端做整個狗榜 UI）**：狗榜頁 + 人/狗維度切換 + 朋友/全 app tab + 設定頁可見度三選一（public/friends/off）+ 狗榜 row 視覺（頭像真照片、breed/age chip？）+ 沿用 glow / reduced-motion + 響應式 + a11y + i18n。資料 wiring（讀 dogLeaderboards、依好友 filter、寫 leaderboardVisibility）也在本次範圍。
- **→ PM（後續）**：等 user 對加法係數 0.4 最終確認（可不擋）；P0 穩定後排入 sprint，從 READY-FOR-DEV 轉「進行中」。

---

## ✅ Backend 已交付（iOS Backend session 2026-05-31 — commit 770d9cb）

**Deployed to `mango-pet-app`**：functions（aggregateLeaderboards 更新 + recomputeDogLeaderboards / recomputeDogLeaderboardsOnDelete / syncDogEntryVisibility 新建）、firestore:rules、firestore:indexes 全部 deploy 成功；index 全 READY；prod e2e 實測 PASS（personal-mode 測試 walk → 即時建 dog entry → 刪 walk → entry 移除）。

### Feature Builder 可直接用

- **讀取**：`dogLeaderboards/{period}/entries/{petId}`，`period ∈ { weekly_YYYY-Www, monthly_YYYY-MM, all_time }`（與人榜同 period key 格式）。登入者皆可讀；client 只能讀不能寫。
- **Entry 形狀最終版**（型別 `DogLeaderboardEntry` @ `apps/web/src/lib/types.ts`）：
  `petId, petName, petPhotoURL(string|null), breed(string|null), species, ownerUid, ownerName, familyId(string|null), totalScore, totalDistanceKm, totalDurationMin, walkCount, streakDays, ownerVisibility('public'|'friends'|'off'), updatedAt, lastUpdatedAt?, previousRank?`。
- **全 app tab**：filter `ownerVisibility === 'public'`。
- **朋友 tab**：client filter `ownerUid ∈ {自己 ∪ listFriends()}` 且 `ownerVisibility ∈ {'public','friends'}`。
- **永遠看得到自己的狗**：reader 對 `ownerUid === 自己` 的 entry 不套 visibility filter（即使 `'off'`）。
- **glow / rank**：沿用人榜機制，`lastUpdatedAt` + `previousRank` 已寫。
- **設定頁**：寫 `users/{uid}.leaderboardVisibility`（`'public'|'friends'|'off'`，型別 `LeaderboardVisibility`；缺值視為 `'public'`）。改值由 `syncDogEntryVisibility` 自動回寫該 user 所有 dog entry 的 `ownerVisibility` — Feature Builder 不需手動回寫。
- **personal-mode 狗**：已納入（`familyId` 可為 null）。

### 注意 / 已知行為
- **歷史分數不 recompute**：dog entry 的 `totalScore` 來自各 walk **存好的** `score`。新公式（dogFactor）只影響**新 walk**；舊 walk 維持舊分（已拍板漂移）。
- **公式向後相容語意**：無 birthday/breed → age/breed 因子 = 1.0（不貢獻）；但 weight 項本身也乘了係數 0.4，故小型犬距離乘數 2.0→1.40（spec 示例表 + 已接受副作用）。**非**「與舊公式數值完全相同」。係數 `DOG_FACTOR_COEFFICIENT = 0.4` 抽成常數於 `scoring.ts`，user 要微調改一處即可。
- **人榜未動**：walker functions / 行為 byte-unchanged；walk `score` 公式為兩榜共用，新公式自然流入兩邊（設計如此）。
- **成本**：未開新 scheduled function（dog pass 併入現有 cron）。cron 的 dog pass 全掃 `walks`（含 personal-mode）→ Firestore 讀取量比人榜大；即時 trigger 為增量。若日後量大，可改 cron 為「僅變動 pet 增量」（spec ⑤ 提過），目前全掃可接受。
