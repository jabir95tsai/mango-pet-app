...
| `261d588` | chore(routes): `/app/expenses` page replaced with server-side Next.js `redirect('/app/pets')`. Bookmarks + the Bug Hunter `?action=scan` deep-link both survive (no 404, no client flash). ⚠️ **PM audit 2026-05-28 更正**：commit message 聲稱 `expense-summary.tsx` deleted，但實際**沒刪** — 檔案仍在 src/ 且 0 import = dead code（列入 pre-iOS cleanup）。 |
