...
- **🍎 iOS app 戰略 — React Native + Expo + Feature parity 並行 PWA** — [`docs/features/ios-app-strategy.md`](../features/ios-app-strategy.md) **STRATEGY GO**（user 2026-05-28 4 個 strategic decisions confirmed）
  ...
  - 🔀 **P1 重排序（user 2026-06-01）**：實機體感「背景不計時」= 核心缺口 → **P1d 背景 GPS 插隊先做**（遛狗手機常鎖屏/口袋，背景續跑比拍照/彩帶更貼近能不能真用；且背景定位 = App Store 最大審查項，提早做提早暴露風險）。順序改為 **P1d → P1b → P1c**。
  - 👉 **下個動作（user）**：
    - ✅ P1a 簽收完成 → **開 iOS Backend session 起 P1d 背景 GPS**：`expo-location` 背景權限（When In Use → Always）+ Info.plist usage strings + `UIBackgroundModes:location`（Expo config plugin）+ **session-only**（遛狗中才背景定位、結束即停）+ 前景 fallback（Always 被拒退前景）。見 [`ios-p1-walks.md`](../features/ios-p1-walks.md) §背景 GPS + parity §F。
    - ⚠️ P1d 改 native config + 新 dep → iOS Backend 走 **branch + linux rollout gate**（README 規則 4）；且 **App Store 背景定位審查 pre-check**（usage string 充分 + session-only + 不常駐）。
    - 之後 **P1b**（done confetti + 手動補登 + recent「全部」展開）→ **P1c**（拍照 + Storage + 自動發動態 walkId cross-link）。
