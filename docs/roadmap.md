...
- **🍎 iOS app 戰略 — React Native + Expo + Feature parity 並行 PWA** — [`docs/features/ios-app-strategy.md`](../features/ios-app-strategy.md) **STRATEGY GO**（user 2026-05-28 4 個 strategic decisions confirmed）
  ...
  - 👉 **下個動作（user，醒來）**：
    - 先收尾 P1（P1b+P1c 實機驗）→ 然後 **照 [`ios-p2-pets.md`](../features/ios-p2-pets.md) §Launch prompts 順序開 session**：① P2-pre（Backend，shared+i18n，web gate）→ ② P2a（FB 骨架，可並行）→ ③ P2b（svg charts）→ ④ P2c（forms）→ ⑤ P2d（camera，實機驗）
    - 每碰新 dep 的 sub-phase 走 branch + linux rollout gate（規則 4）;P2d 須實機（相機）
