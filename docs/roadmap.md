...
- **🍎 iOS app 戰略 — React Native + Expo + Feature parity 並行 PWA** — [`docs/features/ios-app-strategy.md`](../features/ios-app-strategy.md) **STRATEGY GO**（user 2026-05-28 4 個 strategic decisions confirmed）
  ...
  - 👉 **下個動作（user）**：
    - ✅ P1a + P1d 簽收 → **開 iOS Feature Builder session 起 P1b**：done screen（confetti + 達標變體 + 存檔 + notes）+ 手動補登 dialog（isManual=true）+ recent「全部」展開。service `stop()` 回傳 final state（path/distance/duration）直接餵 `createWalk`。
    - 之後 **P1c**（拍照 + Storage + 自動發動態 walkId cross-link）。
    - **follow-up（async-storage 已裝）**：P1a active-pet 持久化現在有 AsyncStorage 可用 → 另開小 task 收（low pri）。
    - ⚠️ P1b/P1c 若引入新 RN 套件（confetti lib / expo-camera 等）→ iOS Backend 走 **branch + linux rollout gate**（規則 4）。
