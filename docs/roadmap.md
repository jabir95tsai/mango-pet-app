...
- **🍎 iOS app 戰略 — React Native + Expo + Feature parity 並行 PWA** — [`docs/features/ios-app-strategy.md`](../features/ios-app-strategy.md) **STRATEGY GO**（user 2026-05-28 4 個 strategic decisions confirmed）
  ...
  - 🚧 **P1a Walks 核心 loop screens CODE COMPLETE 待實機簽收（`a02289c`）** → [`ios-p1a-walks-screens.md`](../features/ios-p1a-walks-screens.md)
    - WalksHome（dial + week strip + 走路狗 + pet picker + CTA）+ 前景 GPS tracking（timer+距離+stop）+ stop→`createWalk` 寫 `walks/{walkId}`（score 走 shared-business，接同 leaderboard trigger）
    - 純 `apps/ios/**`，未碰 web/dep/lockfile；`apps/ios` tsc pass
    - parity §A P1 四列 → 🟡（screens 到位；待 macOS/EAS 走一趟 walk → doc 落地 + leaderboard 反應 才升 ✅）
    - **iOS Backend follow-up**（surfaced）：(a) `computeStreak` + 今日/本週統計 pure helpers 收進 `@mango/shared-business`（目前 web/iOS verbatim 兩份，drift 風險）；(b) active-pet 持久化要 `AsyncStorage`（新 dep → branch+linux gate）
    - **iOS UI/UX follow-up**：dial 平滑 arc（`react-native-svg` 新 dep → Backend gate）+ Reanimated 走路狗 + reduced-motion + done 慶祝對齊 web emerald
  - 👉 **下個動作（user）**：
    - **macOS/EAS 跑 dev build 走一趟** → 簽收 P1a milestone（walk doc 落地 + leaderboard 動）→ 回報 iOS PM 升 parity §A P1 四列 ✅
    - 之後 **P1b**（done confetti + 手動補登 + recent「全部」展開）→ **P1c**（拍照 + Storage + 自動發動態 walkId cross-link）→ **P1d**（背景 GPS，排拍照後留審查 buffer）
    - ⚠️ P1b–P1d 會引入新 RN 套件（svg / Reanimated / expo-camera / AsyncStorage / 背景 location）→ 每個新 dep 由 iOS Backend 走 **branch + linux rollout gate**（README 規則 4；Windows 本地 build 過 ≠ App Hosting 過）
