...
- **🍎 iOS app 戰略 — React Native + Expo + Feature parity 並行 PWA** — [`docs/features/ios-app-strategy.md`](../features/ios-app-strategy.md) **STRATEGY GO**（user 2026-05-28 4 個 strategic decisions confirmed）
  ...
  - 👉 **下個動作（user）**：
    - ✅ P0 完成 + P1 spec + **P1a backend 前置 merged** → 進入 **P1a screens**
    - **開 iOS Feature Builder session 串 WalksHome**：dial + week strip + 走路狗 + pet picker + 接 P1a 的前景 GPS service + 核心追蹤 loop（接 `@mango/shared-business`）→ 實機走一趟驗 walk doc 落地 + leaderboard 有動
    - 之後 P1b 結束+補登 → P1c 拍照+自動分享 → P1d 背景 GPS（排拍照後，留審查 buffer）
    - ⚠️ 任何改 `apps/web` / dep / lockfile 的 commit 前：`npm run -w apps/web build` 自驗 + 走 branch + linux rollout gate（README 規則 4）
