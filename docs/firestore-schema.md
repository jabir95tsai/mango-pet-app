...
>
> 最後校對：2026-05-23（Backend session — friends-search-lowercase Phase 1–4; displayNameLower shadow field + backfill audit collection）
...
- **legacy `users/{uid}/{pets,reminders,walks,expenses,pets/{petId}/healthRecords}` 已 retired**（2026-05-23, #6 cleanup spec）— 資料已清空（audit doc `legacyCleanups/2026-05-23T14-01-53-045Z`）、rules legacy match blocks 移除、client lib 的 5 個 `migrateLegacy*ToFamily()` helper 移除。
...
deletedAccounts/{id}                              — user 帳號刪除 audit（admin-only；deleteUserAccount callable 寫入）
legacyCleanups/{id}                               — legacy 路徑清理 audit（admin-only；cleanupLegacyPaths callable + admin script 寫入）
displayNameLowerBackfills/{id}                    — displayNameLower 補欄 audit（admin-only；backfillDisplayNameLower callable + admin script 寫入）
```

### Retired（資料 + rules + lib helper 都已移除）

```
users/{uid}/pets/{petId}                              — retired 2026-05-23
users/{uid}/pets/{petId}/healthRecords/{recordId}     — retired 2026-05-23
users/{uid}/reminders/{reminderId}                    — retired 2026-05-23
users/{uid}/walks/{walkId}                            — retired 2026-05-23
users/{uid}/expenses/{expenseId}                      — retired 2026-05-23
```

Spec：[features/legacy-path-cleanup.md](features/legacy-path-cleanup.md)。Audit doc：`legacyCleanups/2026-05-23T14-01-53-045Z`（real run，2 pets + 1 walk + 1 reminder + 1 expense 跨 2 個 user 被刪）。

### 預留 collection（rules 已開、callable 尚未寫入）

| 路徑 | 用途 | 來源 |
|---|---|---|
| `families/{familyId}/migrations/{migrationId}` | 家庭層級 callable migration 的 audit log（B3 importPersonalToFamily / B4 pet-merge / 任何未來重大 migration） | rules 已加，等 B3 callable 動工才會有實際寫入 |
...
| `displayNameLower` | upsertUser（create / displayName 變更 / 缺欄位防呆 backfill） | `displayName.trim().toLowerCase()`；`searchUsers` 用它做 case-insensitive prefix match。歷史 docs 由 `displayNameLowerBackfills/2026-05-23T14-52-04-246Z` 補齊 |
...
所有 family-scoped collection 都只寫 top-level，且 legacy 路徑已 retired（2026-05-23，spec [legacy-path-cleanup.md](features/legacy-path-cleanup.md)）：

| 集合 | Client lib 路徑 | Legacy 路徑 |
|---|---|---|
| `pets` | top-level only | ❌ retired |
| `reminders` | top-level only | ❌ retired |
| `walks` | top-level only | ❌ retired |
| `expenses` | top-level only | ❌ retired |
| `pets/healthRecords` | top-level only | ❌ retired |

歷史備註：`src/components/family/family-provider.tsx` 過去在 `(uid, familyId)` 第一次載入時跑 5 個 `migrateLegacy*ToFamily()` helper（idempotent，存 `localStorage["mango.migrated.{uid}.{familyId}"]="1"` 避免重跑）。2026-05-23 連 helper + localStorage 寫入都移除。仍存留在 user device localStorage 的舊 marker 是無害 no-op。

⚠ 殘留問題：legacy migration 期間因 auto-create-family 重複觸發，部分使用者 top-level `pets/{petId}` 有 `(name, species, birthday)` 重複條目（俗稱「兩隻 Mango」）。dedupe migration（原 roadmap #4）已被 cancel；現存的 top-level 重複條目得手動於 console 刪除，或未來重新評估時再規格化。Legacy cleanup 不會新增或修復這類重複條目，只是把舊的 legacy 殘留資料端清掉。
